...
class DataProcessor():
  def __init__(self, model = "ResNet", min_height = 200, min_width = 200, normalise = False, rotate = False, 
  ...
  def normalisation(self, image, mode = ""):
    ...
    if mode == "z_norm":
      ...
      if self.model == "ResNet":
        #imstack = np.hstack((image, resnet50.preprocess_input(image)))
        #cv2_imshow(imstack)
        n_im = resnet50.preprocess_input(image)
